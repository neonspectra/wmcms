...
### Gotchas
- If you want to change the `background-color` css style, you will notice that it probably won't work if you set it just in `/config/main.css`. That's because it's also explicity set in `/config/headers.html` and for all the files in `/config/errors`. Setting the background colour inline in the html body of these pages is important because it prevents users from getting a splash of white (if your site has a dark background) while the linked stylesheet is still loading in the user's browser.
